...
export function HomeHero({ badge, title, subtitle, images, stats = [] }: Props) {
  ...
  return (
    <section id="beranda" className="relative overflow-hidden bg-canvas text-ink">
      {/* Grid pattern */}
      <div
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute left-1/2 top-1/3 h-[36rem] w-[36rem] -translate-x-1/2 rounded-full bg-brand/20 blur-[120px] dark:bg-brand/20" />
        <div className="absolute -left-20 top-20 h-72 w-72 rounded-full bg-brand-strong/10 blur-3xl" />
        <div className="absolute -right-20 top-40 h-72 w-72 rounded-full bg-brand/10 blur-3xl" />
      </div>
      ...
      <div className="relative mx-auto max-w-5xl px-4 pt-28 pb-0 text-center md:pt-32">
        <RevealContainer className="flex flex-col items-center">
          <RevealItem>
            <span className="inline-flex items-center gap-2 rounded-full border border-brand/40 bg-brand/10 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.2em] text-brand-text dark:border-brand/30 dark:text-brand-text">
          ...
          <RevealItem>
            <h1 className="mt-6 font-heading text-4xl font-extrabold leading-[1.05] tracking-tight md:text-6xl">
              {title ?? (
                <>
                  Cetak Generasi{" "}
                  {/* Ramp perunggu→emas. Ujung paling terang berhenti di #D97706
                      (3.5:1 di atas krem) supaya judul tetap lolos AA teks besar. */}
                  <span className="bg-gradient-to-r from-[#92400E] via-[#B45309] to-[#D97706] bg-clip-text text-transparent dark:from-[#FDE68A] dark:via-[#FBBF24] dark:to-[#FCD34D]">
          ...
          <RevealItem className="mt-7 flex flex-wrap items-center justify-center gap-3">
            <Link href="/ppdb">
              <Button size="lg" className="group gap-2 rounded-full bg-brand px-7 font-semibold text-brand-ink shadow-lg shadow-brand/25 hover:bg-brand-strong">
                Daftar Sekarang
            ...
            <Link href="/jurusan">
              <Button size="lg" variant="outline" className="gap-2 rounded-full border-slate-300 bg-white/90 px-7 text-slate-800 hover:bg-slate-100 dark:border-white/20 dark:bg-white/5 dark:text-white dark:hover:bg-white/10">
                <GraduationCap className="h-4 w-4" />Program Keahlian
              </Button>
            </Link>
          </RevealItem>
        </RevealContainer>
          ...
          >
            {pics.map((src, i) => {
              ...
              return (
                ...
                >
                  <div className="overflow-hidden rounded-3xl bg-gradient-to-b from-brand to-brand-strong p-1.5 shadow-2xl shadow-anchor/40 ring-1 ring-white/10">
            ...
            <div className="pointer-events-none absolute -bottom-1 left-0 right-0 h-24 bg-gradient-to-t from-canvas to-transparent" />
          </RevealContainer>
        )}
          ...
          <RevealContainer className="relative z-30 mt-8 grid grid-cols-2 gap-3 pb-12 sm:grid-cols-4">
            {stats.slice(0, 4).map((s) => (
              <RevealItem key={s.id} className="rounded-2xl border border-slate-200 bg-white/80 px-3 py-4 shadow-sm dark:border-white/10 dark:bg-white/5 dark:shadow-none dark:backdrop-blur">
                <div className="font-heading text-2xl font-bold text-brand-text md:text-3xl">{s.value}</div>
