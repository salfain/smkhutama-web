    ...
    <>
      ...
      <section className="relative overflow-hidden mesh-bg text-white">
        <div className="pointer-events-none absolute inset-0">
          <div className="animate-float-slow absolute -left-16 top-0 h-56 w-56 rounded-full bg-brand-strong/30 blur-3xl" />
          <div className="animate-float-slower absolute right-0 bottom-0 h-64 w-64 rounded-full bg-brand-strong/30 blur-3xl" />
        </div>
      ...
      {/* Content */}
      <section className="bg-canvas-alt pb-16">
        <div className="mx-auto max-w-6xl px-4">
          <Link href="/berita" className="inline-flex items-center gap-1.5 text-sm font-medium text-brand-text hover:text-brand-text mb-8 transition-colors">
          ...
          <div className="grid gap-10 lg:grid-cols-3">
            ...
            <article className="lg:col-span-2">
              ...
              <div
                className="news-content max-w-none text-ink leading-relaxed"
            ...
            <aside className="lg:col-span-1">
              <div className="sticky top-24 rounded-2xl border border-hairline bg-canvas p-6">
                <h3 className="font-heading text-base font-bold text-ink mb-4">
                  Berita Lainnya
                </h3>
                {related.length === 0 ? (
                  <p className="text-sm text-ink-soft">Belum ada berita lain.</p>
                ) : (
                      ...
                      <Link key={item.id} href={`/berita/${item.slug}`} className="group block">
                        <div className="flex gap-3">
                          {item.imageUrl ? (
                            ...
                            <div className="h-16 w-16 rounded-lg bg-gradient-to-br from-anchor-soft to-anchor flex items-center justify-center flex-shrink-0">
                              <Newspaper className="h-5 w-5 text-white/60" />
                            </div>
                          )}
                          <div className="min-w-0">
                            <h4 className="text-sm font-semibold text-ink line-clamp-2 group-hover:text-brand-text transition-colors">
                              {item.title}
                            </h4>
                            <time className="text-[11px] text-ink-soft mt-1 block">
