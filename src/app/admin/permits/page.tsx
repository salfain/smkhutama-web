  ...
  return (
    <div className="p-4 md:p-6 lg:p-8">
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-bold text-gray-900 dark:text-white">Rekap Izin Siswa</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400">Catatan keluar/masuk dari guru piket, ditampilkan baca saja.</p>
        </div>
      ...
      <div className="overflow-x-auto rounded-xl border bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <table className="w-full min-w-[860px] text-sm">
          <thead className="bg-gray-50 text-left text-xs text-gray-500 dark:bg-slate-950 dark:text-gray-400">
            <tr><th className="px-4 py-3">Siswa</th><th className="px-4 py-3">Alasan</th><th className="px-4 py-3">Keluar</th><th className="px-4 py-3">Kembali</th><th className="px-4 py-3">Status</th></tr>
          </thead>
          <tbody className="divide-y dark:divide-slate-800">
            {records.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-10 text-center text-gray-400">Tidak ada catatan pada filter ini.</td></tr>
            ) : records.map((record) => (
              <tr key={record.id}>
